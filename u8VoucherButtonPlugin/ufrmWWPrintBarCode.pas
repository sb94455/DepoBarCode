...
uses
  ...
  frxExportRTF, frxExportXLS,  frxGradient, frxBarcode, frxOLE,
  ...
  frxADOComponents, DBClient, frxExportPDF;

type
  TfrmWWPrintBarCode= class(TForm)
...
var
  frmWWPrintBarCode: TfrmWWPrintBarCode;
...
procedure TfrmWWPrintBarCode.SetVouchType(Value: string);
begin
  FVouchType := Value;
  cxcbbReportQuery;
end;

procedure TfrmWWPrintBarCode.frxReport1Preview(Sender: TObject);
var
...
procedure TfrmWWPrintBarCode.btnPrintClick(Sender: TObject);
var
  ms: TMemoryStream;
  i: Integer;
begin
...
procedure TfrmWWPrintBarCode.CopySelRowsToCds(ViewSoure: TcxGridDBTableView; cdsDes: TClientDataSet);
var
  i, j: Integer;
...
function TfrmWWPrintBarCode.frxDesigner1SaveReport(frxReport1: TfrxReport; SaveAs: Boolean): Boolean;
var
  ms: TMemoryStream;
begin
  if SaveAs then
...
procedure TfrmWWPrintBarCode.FormShow(Sender: TObject);
var
  i: Integer;
begin
...
procedure TfrmWWPrintBarCode.btnExitClick(Sender: TObject);
var
  i: Integer;
begin
...
procedure TfrmWWPrintBarCode.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  Action := caFree;
end;

procedure TfrmWWPrintBarCode.FormKeyDown(Sender: TObject; var Key: Word; Shift: TShiftState);
begin
...
procedure TfrmWWPrintBarCode.qryReportBeforeOpen(DataSet: TDataSet);
begin
  (DataSet as TADOQuery).SQL.Text := Format('select * from SZHL_Report where VouchType=''%s''', [VouchType]);
end;

procedure TfrmWWPrintBarCode.qryVouchsBeforeOpen(DataSet: TDataSet);
begin
  (DataSet as TADOQuery).SQL.Text := Format('select * from SZHL_OM_MOMainView Where %s=%s', ['VouchId', keyValue]);
end;

procedure TfrmWWPrintBarCode.qryVouchsAfterOpen(DataSet: TDataSet);
begin
  FillGridViewColumns(cxgrdbtblvwVouchRows, 'SZHL_OM_MOMainView');
...
procedure TfrmWWPrintBarCode.conMainBeforeConnect(Sender: TObject);
begin
  conMain.ConnectionString := strDBConnectString;
end;

procedure TfrmWWPrintBarCode.btnReportManageClick(Sender: TObject);
var
  i: Integer;
begin
...
procedure TfrmWWPrintBarCode.cxcbbReportQuery;
var
  i: Integer;
begin
  qryReport.Close;
...
procedure TfrmWWPrintBarCode.cxcbbReportPropertiesChange(Sender: TObject);
begin
  strCbbItemIndexValue := VarToStr(cxcbbReport.EditValue);
end;

procedure TfrmWWPrintBarCode.actCreateQRExecute(Sender: TObject);
var
  strMsg: string;
begin
...
procedure TfrmWWPrintBarCode.actCreateAllQRExecute(Sender: TObject);
var
  bkVouchs: string;
  strMsgOk: string;
  strMsg: string;
...
procedure TfrmWWPrintBarCode.actDelQRExecute(Sender: TObject);
begin
  if dsQRCode.Eof then
    Exit;
end;

procedure TfrmWWPrintBarCode.actNewMQRExecute(Sender: TObject);
begin
  if qryVouchs.Eof then
    Exit;
...
procedure TfrmWWPrintBarCode.actFiltExecute(Sender: TObject);
begin
...
procedure TfrmWWPrintBarCode.dsQrCodeByInventoryBeforeOpen(DataSet: TDataSet);
begin
...
procedure TfrmWWPrintBarCode.dsQrCodeByInventoryAfterOpen(DataSet: TDataSet);
begin
  FillGridSelColumn(cxgrdbtblvwInverntory, 'autoid');
  FillGridViewColumns(cxgrdbtblvwInverntory, 'SZHL_QRCodeByInventory');
end;

procedure TfrmWWPrintBarCode.actAddByInventoryExecute(Sender: TObject);
begin
  with TfrmCreateQR.Create(Self) do
  try
    VouchType := Self.VouchType;
...
procedure TfrmWWPrintBarCode.dsQRCodeAfterOpen(DataSet: TDataSet);
begin
  FillGridSelColumn(cxgrdbtblvwQRCodes, 'autoid');
  FillGridViewColumns(cxgrdbtblvwQRCodes, 'SZHL_QRCode', False);
end;

procedure TfrmWWPrintBarCode.actSelAllExecute(Sender: TObject);
var
  i: integer;
begin
...
procedure TfrmWWPrintBarCode.actUnSelAllExecute(Sender: TObject);
var
  i: integer;
begin
  for i := 0 to cxgrdbtblvwQRCodes.DataController.RowCount - 1 do
...
procedure TfrmWWPrintBarCode.actSelUnSelExecute(Sender: TObject);
var
  i: integer;
  blValue: Boolean;
begin
...
procedure TfrmWWPrintBarCode.qryVouchsAfterScroll(DataSet: TDataSet);
begin
...
procedure TfrmWWPrintBarCode.cxgrdbtblvwQRCodesCellClick(Sender: TcxCustomGridTableView; ACellViewInfo: TcxGridTableDataCellViewInfo; AButton: TMouseButton; AShift: TShiftState; var AHandled: Boolean);
var
  i, iRowIndex: Integer;
...
procedure TfrmWWPrintBarCode.frxReport1PrintReport(Sender: TObject);
begin
  rpPrinted := True;
end;

procedure TfrmWWPrintBarCode.btnPrintVouchsClick(Sender: TObject);
var
  ms: TMemoryStream;
  i: Integer;
begin
