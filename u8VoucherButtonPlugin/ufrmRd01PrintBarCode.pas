...
begin
  ...
  try
    ...
    if ViewSoure.DataController.FilteredRecordCount > 0 then  //过滤后有记录数
    begin
      for i := 0 to ViewSoure.ViewData.RowCount - 1 do//以行数循环
      begin
        ViewSoure.DataController.DataSet.RecNo := ViewSoure.ViewData.Rows[i].RecordIndex + 1; //定位数据集记录指针
        ViewSoure.DataController.DataSet.EDIT;
        ViewSoure.DataController.DataSet.FieldByName('printsign').Value := format('第%d次打印', [1 + j]);
...
//  ShowMessage(strJasonData);
  ...
  begin
    try
      ...
      if trim(strMsg) = '' then
        MessageBox(Application.handle, '合并成功！', '提示', MB_ICONINFORMATION + MB_OK)
      else
        MessageBox(Application.Handle, PChar('合并失败.'#10#13 + strMsg), '错误', MB_ICONERROR);
    finally
      Free
    end;
  end;
