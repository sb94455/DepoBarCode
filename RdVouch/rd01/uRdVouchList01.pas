...
uses
  ...
  FireDAC.Comp.Client{$IFDEF ANDROID} , FMX.platform.Android {$ENDIF}, Soap.SOAPHTTPClient,
  Xml.XMLDoc, Xml.XMLIntf, FireDAC.Stan.StorageBin, FMX.Bind.Grid, System.Bindings.Outputs,
  ...
  FMX.Edit, FMX.ListBox, UnitLib ;

type
  Tfrm_RdVouchList01 = class(TBaseFrm)
...
var
  frm_RdVouchList01: Tfrm_RdVouchList01;

implementation

uses
  PurchaseInService, uRdVouch01, uSZHLFMXEdit;
{$R *.fmx}

procedure Tfrm_RdVouchList01.btn_ViewVouchClick(Sender: TObject);
begin
...
procedure Tfrm_RdVouchList01.ClearBtnClick(Sender: TObject);
begin
  inherited;
  ClearChildCtrl(lyt_Main, False);
end;

procedure Tfrm_RdVouchList01.DeleteBtnClick(Sender: TObject);
var
  // tmpHTTPRIO           : THTTPRIO;
...
begin
  if FDM_VouchList.RecordCount > 0 then
    if FDM_VouchList.FieldByName(VouObj.DesMainKeyField).AsString.Trim <> '' then
    begin
//     if MessageDlg('', TMsgDlgType.mtInformation,) then

      try
        begin
          // tmpHTTPRIO := THTTPRIO.Create(Nil);
          Service_Data := GetPurchaseInServiceSoap(False, PubVar_WebServiceUrl + Const_Url_Def_PurchaseIn, u8dm.HTTPRIO1);
...
procedure Tfrm_RdVouchList01.FDM_VouchListAfterOpen(DataSet: TDataSet);
begin
  inherited;
  DM_Global.FormatGrid_byAD(VouObj.ListTableDef, '', FDM_VouchList, LinkGridToData_VouchList);
end;

procedure Tfrm_RdVouchList01.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  inherited;
  FVouchObj := nil;
  frm_RdVouchList01 := nil;
end;

procedure Tfrm_RdVouchList01.FormCreate(Sender: TObject);
begin
  inherited;
  FVouchObj := TSZHL_Vouch.Create;
  with FVouchObj do
  begin
    ListTable := 'SZHL_Wss_V_PurchaseIn_List';
    ListTableDef := '采购入库单列表视图';

    DesMainTable := 'RdRecord01';
    DesMainTableDef := '采购入库单主表';
    DesMainKeyField := 'ID';

    DesSubTable := 'RdRecords01';
    DesSubTableDef := '采购入库单子表';
    DesSubKeyField := 'Autoid';
    DesSubQtyField := 'iQuantity';
    DesSubLinkSrcFeild := 'iArrsId';
    DesSubCardTable:='采购入库单子表卡片视图';
    DesSubLinkMainFeild := 'ID';

    SrcTable := 'SZHL_Wss_V_PurchaseIn_Src';
    SrcTableDef := '采购入库参照到货单视图';
    SrcKeyField := 'Autoid';
    SrcLinkDesSubField := 'AutoId';
    SrcQtyField := 'iQuantity';
    SrcRmtQtyField := 'fValidInQuan';
    SrcLinkMainField := 'ID'; // PU_ArrivalVouchs.ID,PU_ArrivalVouch.ID
...
procedure Tfrm_RdVouchList01.InitData;
begin
  inherited;
end;

procedure Tfrm_RdVouchList01.LoadVouch(strVouchId: string);
begin
  if not Assigned(frm_RdVouch01) then
  begin
    frm_RdVouch01 := tfrm_RdVouch01.Create(Self);
  end;
  frm_RdVouch01.Tab_Sub.Visible := strVouchId.IsEmpty;


  frm_RdVouch01.VouObj := Self.VouObj;
  with frm_RdVouch01.VouObj do
  begin
    DesMainKey := strVouchId;
    if Prepared then
    begin
      frm_RdVouch01.InitData;
      ShowForm(Self, frm_RdVouch01);
    end;
  end;

end;

procedure Tfrm_RdVouchList01.QueryBtnClick(Sender: TObject);
begin
  inherited;
  RefreshData;
end;

function Tfrm_RdVouchList01.GetSqlWhereText: string;
var
  strSqlstring: string;
...
procedure Tfrm_RdVouchList01.RefreshData;
var
  sqlVouchList: string;
  column: TColumn;
begin
  LinkGridToData_VouchList.Columns.Clear; // 不加这句，新增的calc类型字段会报错找不到，花费1H+
  sqlVouchList := format('select distinct(cCode),cWhCode,dDate,ID from %s where %s order by ccode', [VouObj.ListTable, GetSqlWhereText]);
  DM_Global.ExecSql(FDM_VouchList, sqlVouchList);
end;

procedure Tfrm_RdVouchList01.RefreshDelete;
begin

end;

procedure Tfrm_RdVouchList01.btn_NewVouchClick(Sender: TObject);
begin
  LoadVouch('');
end;
