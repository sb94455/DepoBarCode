...
procedure Tfrm_RdVouch09.fdm_DetailAfterOpen(DataSet: TDataSet);
begin
  inherited;
  DM_Global.FormatGrid_byAD(FVouchObj.DesSubTableDef, '', fdm_Detail, LinkGridToData_Detail);
end;

procedure Tfrm_RdVouch09.btn_BaseBackClick(Sender: TObject);
begin
  inherited;
  frm_RdVouch09 :=nil;
end;

procedure Tfrm_RdVouch09.btn_ClearDetailClick(Sender: TObject);
begin
  inherited;
...
procedure Tfrm_RdVouch09.btn_SaveClick(Sender: TObject);
var
 strsqlmain ,strsqldetail : string;
begin
  inherited;
  strsqlmain := Format('Exec SZHL_UPDATEOTHEROUTMAIN ''%s'',''%s'',''%s'' ',[fdm_main.FieldByName('cWhCode').AsString,fdm_main.FieldByName('cMemo').AsString,fdm_main.FieldByName('cMaker').AsString]);
  DM_Global.ExecuteSql(strsqlmain);
  fdm_detail.First;

  while not fdm_detail.Eof do
  begin
    strsqldetail := Format('Exec SZHL_UPDATEOTHEROUTDETAIL ''%s'',''%s'',''%s'',''%s'' ',[fdm_detail.FieldByName('cInvCode').AsString,fdm_detail.FieldByName('iQuantity').AsString,fdm_detail.FieldByName('cBatch').AsString,fdm_detail.FieldByName('cposition').AsString]);
...
procedure Tfrm_RdVouch09.cbRedClick(Sender: TObject);
begin
...
procedure Tfrm_RdVouch09.DecryptQRCode;
var
  cmd, strDesInvCode ,cBatch ,FirstBatch : string;
  iQuantity   : double;
  SN          : Integer;
  //AutoId, ID : Integer;
  label loopstart;
...
begin
  ...
  //
      ...
      begin
        ...
        fdm_Detail.FieldByName('iQuantity').Value := '';
...
function Tfrm_RdVouch09.GetcWhCode: string;
begin
  Result := '';
...
procedure Tfrm_RdVouch09.FormKeyUp(Sender: TObject; var Key: Word; var KeyChar: Char; Shift: TShiftState);
begin
  inherited;
  if Key = vkF11 then
...
procedure Tfrm_RdVouch09.setDefaulPosition(Value: string);
begin
  FPosition := Value;
  txt_BaseTitle.Text := format('%s  P:%s', [VouObj.DesMainTableDef, Value])
end;

procedure Tfrm_RdVouch09.fdm_DetailAfterPost(DataSet: TDataSet);
begin
  inherited;
  RefreshData;
  DM_Global.FormatGrid_byAD(FVouchObj.DesSubTableDef, '', fdm_Detail, LinkGridToData_Detail);
...
procedure Tfrm_RdVouch09.fdm_DetailBeforePost(DataSet: TDataSet);
var
  strErrMsg: string;
begin
  inherited;
...
//procedure Tfrm_RdVouch09.ShowSubDetail;       //显示subdetail数据
//var
//  strVouchSql, strWhCode, strcInvCode,strsql: string;
//begin
//  strWhCode := fdm_Main.FieldByName('cWhCode').AsString.Trim;
//  strcInvCode := GetValueByKey(FQRCode, 'cInvCode');
//
...
//  end;
//  strVouchSql := format('select *  from InvPositionSum where cInvCode=''%s'' and cWhCode = ''%s'' order by cWhCode,cInvCode,cBatch',[strcInvCode,strWhCode]);
//  DM_Global.ExecSql(fdm_SubDetail, strVouchSql);
//end;

procedure Tfrm_RdVouch09.edt_QRCodeKeyUp(Sender: TObject; var Key: Word; var KeyChar: Char; Shift: TShiftState);
begin
  inherited;
  if Key = 13 then
    try
...
procedure Tfrm_RdVouch09.fdm_MainAfterInsert(DataSet: TDataSet);
begin
  inherited;
...
procedure Tfrm_RdVouch09.fdm_MainAfterOpen(DataSet: TDataSet);
var
  strVouchsSql : string;
begin
  inherited;
...
procedure Tfrm_RdVouch09.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  inherited;
...
procedure Tfrm_RdVouch09.FormCreate(Sender: TObject);
begin
  inherited;
...
procedure Tfrm_RdVouch09.FormShow(Sender: TObject);
var
...
procedure Tfrm_RdVouch09.InitData;
var
  strVouchSql: string;
begin
  inherited;
...
procedure Tfrm_RdVouch09.RefreshData; // 根据Detail汇总到Sub
var
  strVouchsSql: string;
