...
type
  Tfrm_RdVouchList09 = class(TBaseFrm)
...
var
  frm_RdVouchList09: Tfrm_RdVouchList09;
...
procedure Tfrm_RdVouchList09.btn_BaseBackClick(Sender: TObject);
begin
  inherited;
  frm_RdVouchList09 :=nil;
end;

procedure Tfrm_RdVouchList09.btn_NewVouchClick(Sender: TObject);
begin
  inherited;
  LoadVouch('');
end;

procedure Tfrm_RdVouchList09.btn_ViewVouchClick(Sender: TObject);
begin
...
procedure Tfrm_RdVouchList09.ClearBtnClick(Sender: TObject);
begin
  inherited;
  ClearChildCtrl(lyt_Main,False);
end;

procedure Tfrm_RdVouchList09.LoadVouch(strVouchId: string);
begin
  if not Assigned(frm_RdVouch09) then
  begin
    frm_RdVouch09 := Tfrm_RdVouch09.Create(Self);
  end;
 // frm_RdVouch08.Tab_Sub.Visible := strVouchId.IsEmpty;             //
  frm_RdVouch09.VouObj := Self.VouObj;
  with frm_RdVouch09.VouObj do
  begin
    DesMainKey := strVouchId;
    if Prepared then
    begin
      frm_RdVouch09.InitData;
      ShowForm(Self, frm_RdVouch09);
    end;
  end;

end;

procedure Tfrm_RdVouchList09.FDM_VouchListAfterOpen(DataSet: TDataSet);
begin
  inherited;
  DM_Global.FormatGrid_byAD(VouObj.ListTableDef, '', FDM_VouchList, LinkGridToData_VouchList);
end;

procedure Tfrm_RdVouchList09.FormCreate(Sender: TObject);
begin
  inherited;
  ...
  begin
    // MainKey := strVouchId;
    ListTable := 'SZHL_Wss_V_OtherOut_List';
    ListTableDef := '其他出库单列表视图';

    DesMainTable := 'RdRecord09';
    DesMainTableDef := '其他出库单主表';
    DesMainKeyField := 'ID';

    DesSubTable := 'RdRecords09';
    DesSubTableDef     := '其他出库单子表';
...
procedure Tfrm_RdVouchList09.QueryBtnClick(Sender: TObject);
begin
  inherited;
  RefreshData;
end;

procedure Tfrm_RdVouchList09.RefreshData;
var
  sqlVouchList: string;
  column      : TColumn;
begin
...
function Tfrm_RdVouchList09.GetSqlWhereText: string;
var
  strSqlstring: string;
