...
begin
  ...
  begin
    ...
    ListTabledef := '材料出库单列表视图';

    DesMainTable := 'RdRecord11';
    DesMainTableDef := '材料出库单主表';
    DesMainKeyField := 'ID';

    DesSubTable := 'RdRecords11';
    DesSubTableDef := '材料出库单子表';
    DesSubKeyField := 'Autoid';
    DesSubQtyField := 'iQuantity';
    DesSubLinkSrcFeild := 'iMPoIds'; // 对应mom_moallocate.AllocateId
    DesSubCardTable:='材料出库单子表卡片视图';

    SrcTable := 'SZHL_Wss_V_MaterialOut_Src';
    SrcTableDef := '材料出库单参照生产订单用料表视图';
